...
import fr.quiibz.gui.modified.GuiIngame;
import fr.quiibz.gui.modified.GuiIngameMenu;
import fr.quiibz.gui.modified.GuiInventory;
...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    private void startGame() throws LWJGLException, IOException
    {
        ...
        DrakogiaMod.init();
        //PlayerName
        String playerName = this.getMinecraft().getSession().getUsername();        
        Drakogia.setPlayerName(playerName); 
        ...
        //Drakogia
        Display.setTitle("Drakogia, " + Drakogia.getPlayerName());
    }
    ...
    private void processKeyBinds()
    {
        ...
        while (this.gameSettings.keyBindMacro1.isPressed()) {
        	
        	if(settings.getMacro1() == null || settings.getMacro1() == "" || settings.getMacro1() == " ") {
        		
        		this.player.addChatComponentMessage(new TextComponentString("§cTu n'a pas d�fini cette macro !"), true);
        		
        	} else {
        	
        		this.player.sendChatMessage("/" + settings.getMacro1());
        		
        	}
        }
        
        while (this.gameSettings.keyBindMacro2.isPressed()) {
        	
        	if(settings.getMacro2() == null || settings.getMacro2() == "" || settings.getMacro2() == " ") {
        		
        		this.player.addChatComponentMessage(new TextComponentString("§cTu n'a pas d�fini cette macro !"), true);
        		
        	} else {
        	
        		this.player.sendChatMessage("/" + settings.getMacro2());
        		
        	}
        }

		while (this.gameSettings.keyBindMacro3.isPressed()) {
			
			if(settings.getMacro3() == null || settings.getMacro3() == "" || settings.getMacro3() == " ") {
				
				this.player.addChatComponentMessage(new TextComponentString("§cTu n'a pas d�fini cette macro !"), true);
				
			} else {
			
				this.player.sendChatMessage("/" + settings.getMacro3());
