...
public class GuiIngame extends Gui
{
    ...
    public void drawItemInToScreen(ItemStack is, int x, int y, float par3, boolean overlay)
    {
        if (is != null)
        {
    ...
    private String join1 = null;    
    private int timer1 = 0;
    
    private String join2 = null;    
    private int timer2 = 0;
    
    private String join3 = null;    
    private int timer3 = 0;
    
    public void addJoinOrLeaveMessage(String text) {
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (!this.mc.gameSettings.showDebugInfo){
        	...
        	//Drakogia            	
        	DrakogiaMod.getRenderer().renderKeystrokes();
        	...
        	this.drawString(fontrenderer, "§6Drakogia.fr", i - fontrenderer.getStringWidth("§6Drakogia.fr") - 10, 2, 1);
        	this.drawString(fontrenderer, "§7X: §a" + blockpos.getX(), i - fontrenderer.getStringWidth("§eX: " + blockpos.getX()) - 2, 16, 1);
        	this.drawString(fontrenderer, "§7Y: §a" + blockpos.getY(), i - fontrenderer.getStringWidth("§eY: " + blockpos.getY()) - 2, 28, 1);
        	this.drawString(fontrenderer, "§7Z: §a" + blockpos.getZ(), i - fontrenderer.getStringWidth("§eZ: " + blockpos.getZ()) - 2, 40, 1);
        	
        	
        	if(settings.canSeeFPS()) this.drawString(fontrenderer, "§a" + this.mc.getDebugFPS() + " FPS", 2, 2, 0x00000000);
        	if(settings.hasToogleSprint()) {
        		
        		this.drawString(fontrenderer, "§eToogleSprint: §aOui", i - fontrenderer.getStringWidth("§eToogleSprint: §aOui") - 2, 54, 1);
        		
        	} else {
        		
        		this.drawString(fontrenderer, "§eToogleSprint: §cNon", i - fontrenderer.getStringWidth("§eToogleSprint: §cNon") - 2, 54, 1);
        	}
        	...
        	for (int e = 0; e < 4; ++e) {
        	    ...
        	    is = this.mc.player.inventory.armorItemInSlot(4 - e - 1);
        	    
        	if (is == null)
        		
        	continue;
        	
        	    if(settings.canSeeArmorHUD()) drawItemInToScreen(is, 2, e * 17 + 30, 1, true);
        	    
        	    RenderHelper.disableStandardItemLighting();
        	    
        	    int damageToDisplay = is.getMaxDamage() - is.getItemDamage();
        	    
        	    if(damageToDisplay != 0 && settings.canSeeDurablity()) this.drawString(fontrenderer, "§7" + damageToDisplay, 20, e * 17 + 35, 0);        	    
        	}       
        	...
        	if(join1 != null) {
        		...
        		if(join1.contains("+")) {
        			...
        			message = join1.replace("[+] ", "");            		
            		msg = "§7[§a+§7] §a" + message;
            		
        		} else if(join1.contains("-")) {
        		...
        		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        		 ...
        		 int alpha = timer1 < 10 ? ((int) (timer1 * 25)) : 255;

        		 // Couleur de base du message (simplement blanc auquel on a ajouté la transparence)
        		 int color = 0xffffff | ((alpha & 255) << 24);
      
        		 this.drawString(fontrenderer, msg, i - fontrenderer.getStringWidth(msg) - 5, j - 50, color);
        	...
        	if(join2 != null) {
        		...
        		if(join2.contains("+")) {
        			...
        			message = join2.replace("[+] ", "");            		
            		msg = "§7[§a+§7] §a" + message;
            		
        		} else if(join2.contains("-")) {
        		...
        		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        		 ...
        		 int alpha = timer2 < 10 ? ((int) (timer2 * 25)) : 255;

        		 // Couleur de base du message (simplement blanc auquel on a ajouté la transparence)
        		 int color = 0xffffff | ((alpha & 255) << 24);
        		 
        		 this.drawString(fontrenderer, msg, i - fontrenderer.getStringWidth(msg) - 5, j - 65, color);
        	...
        	if(join3 != null) {
        		...
        		if(join3.contains("+")) {
        			...
        			message = join3.replace("[+] ", "");            		
            		msg = "§7[§a+§7] §a" + message;
            		
        		} else if(join3.contains("-")) {
        		...
        		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        		 ...
        		 int alpha = timer3 < 10 ? ((int) (timer3 * 25)) : 255;

        		 // Couleur de base du message (simplement blanc auquel on a ajouté la transparence)
        		 int color = 0xffffff | ((alpha & 255) << 24);
        		 
        		 this.drawString(fontrenderer, msg, i - fontrenderer.getStringWidth(msg) - 5, j - 80, color);
    ...
    public void updateTick()
    {
    	...
    	// Si on est en train d'afficher un message...
    	if (this.timer1 > 0)
    	{
